...
이러한 명령형 프로그래밍은 "how to solve it", 
즉 어떻게 그것을 해결할 것인가에 관심이 있다. 
...
선언형 프로그래밍은 무엇을(What) 할 것인가에 가깝다. 
현실을 예로 들어 설명해보자.
첫번째 예시
...
두 번째 예시
낯선 사람이 당신에게 OO호텔이 어디 있는지를 묻는다. 
당신은 그곳이 어디 있는지 잘 안다. 이때 당신은 이렇게 말한다.
...
public static void main(String[] args) {
		...
		//선언형(자바에서 stream은 선언형 프로그래밍, 함수형 프로그래밍)
		int result2= 0;
		result2 = list.stream() //통로로 쭉 나열
				  .filter(n -> n % 2 == 0) //필터링 : 2로 나눈게 0인 것들만 통과
				  .mapToInt(Integer:: intValue) //정수로 바꿔서
				  .sum(); // 누적해서 더해서 반환
	}
	
	
	함수형 프로그래밍은 선언형 프로그래밍을 따르는 대표적인 패러다임(사고 방식)이다
...
public class Ex01_Function_Program {
	public static void main(String[] args) {
		...
		// 선언형(자바에서 stream은 선언형 프로그래밍, 함수형 프로그래밍)
		int result2 = 0;
		// 로직은 추상화(내부 구현 작동은 모름)
		// 자바는 스트림을 제공한다(표현은 람다식 사용함)
		result2 = list.stream() // 통로로 쭉 나열
				.filter(n -> n % 2 == 0) // 필터링 : 2로 나눈게 0인 것들만 통과
				.mapToInt(Integer::intValue) // 정수로 바꿔서
				.sum(); // 누적해서 더해서 반환
		...
		// 선언형(함수형) 프로그래밍의 장점 : 
		// 		코드의 가독성 향상, 일관성 유지(누가 짜도 동일한 코드) 
	}
}
